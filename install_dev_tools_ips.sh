...
if [ -z "$_LOG_ACTIVE" ]; then
    _LOG_ACTIVE=1 exec /bin/sh "$0" "$@" 2>&1 | tee -i setup_ips.log
    exit $?
fi
...
VENV_DIR="ml_venv"
MIN_DISK_MB=5120  # 5 GB для Python та пакетів на IPS-базованих системах

# ------------------------------------------
# INFRASTRUCTURE AS CODE (IaC) ANCHOR
# ------------------------------------------
printf "\n\033[1;35m========================================================================================================\033[0m\n"
printf "\033[1;35m[ENTERPRISE IPS PROVISIONER] Детерміністичне IaC-розгортання для IPS-базованих систем.\033[0m\n"
printf "\033[1;35m                             Architect: IRONKAGE\033[0m\n"
printf "\033[1;35m--------------------------------------------------------------------------------------------------------\033[0m\n"
printf "\033[1;35m                             ОС: SunOS 5.11 (Oracle Solaris 11.4+ / OpenIndiana / OmniOS)\033[0m\n"
printf "\033[1;35m                             Стек: Image Packaging System (pkg), Native Zones, Python\033[0m\n"
printf "\033[1;35m                             Гарантує транзакційну цілісність пакетів (FMRI) виключно на x86_64 (amd64).\033[0m\n"
printf "\033[1;35m========================================================================================================\033[0m\n\n"
...
# ==========================================
print_msg "1. Підготовка: Перевірка архітектури (IPS / pkg)..."

# 1.1 Захист операційної системи
if [ "$(uname -s)" != "SunOS" ]; then
    print_err "Цей скрипт призначений виключно для SunOS-систем (Solaris / OpenIndiana)."
...
print_succ "ОС: SunOS (IPS архітектура)"
...
if ! command -v pkg >/dev/null 2>&1; then
    print_err "Менеджер пакетів IPS ('pkg') не знайдено!"
    if command -v pkgin >/dev/null 2>&1; then
        printf "\033[1;33mℹ️ Знайдено pkgin. Будь ласка, запустіть install_dev_tools_illumos.sh\033[0m\n"
    fi
    exit 1
fi
print_succ "Знайдено транзакційний менеджер: IPS (pkg)"

# 1.4 Перевірка ізоляції
ZONENAME=$(zonename)
if [ "$ZONENAME" = "global" ]; then
    printf "\033[1;33m⚠️ УВАГА: Ви знаходитесь у Global Zone.\033[0m\n"
    printf "\033[1;33m   В Oracle Solaris / OpenIndiana це допускається, але для ML-середовищ\033[0m\n"
    printf "\033[1;33m   рекомендується створити окрему Non-Global Zone (ipkg/solaris-kz).\033[0m\n"
else
    print_succ "Локація: Ізольована Zone ($ZONENAME)"
fi
...
# ==========================================
print_msg "2. Оновлення репозиторіїв IPS..."
$SUDO_CMD pkg refresh

# ==========================================
# 3. ВСТАНОВЛЕННЯ PYTHON
# ==========================================
print_msg "3. Розгортання Python $PYTHON_MIN_MAJOR.$PYTHON_MIN_MINOR+ ..."
if [ -z "$PYTHON_BIN" ]; then
    printf "Пошук доступних версій Python у репозиторії (FMRI)...\n"
    if pkg info -r "pkg:/runtime/python-39" >/dev/null 2>&1; then
        $SUDO_CMD pkg install --accept "pkg:/runtime/python-39"
        PYTHON_BIN="python3.9"
    elif pkg info -r "pkg:/runtime/python-310" >/dev/null 2>&1; then
        $SUDO_CMD pkg install --accept "pkg:/runtime/python-310"
        PYTHON_BIN="python3.10"
    else
        print_err "У базовому IPS-репозиторії не знайдено Python 3.9+. Перевірте налаштування publisher."
        exit 1
    fi
fi
